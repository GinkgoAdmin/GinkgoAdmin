...
export const useAuthStore = defineStore('auth', {
  ...
  actions: {
    ...
    /** 保存刷新令牌与 access token 过期时间（用于静默续期，如大屏长时间挂屏） */
    setRefreshToken(refreshToken?: string, expiresAt?: string) {
      if (refreshToken !== undefined) this.refreshToken = refreshToken || ''
      if (expiresAt !== undefined) this.tokenExpiresAt = expiresAt || ''
      this.saveToStorage()
    },
    ...
    saveToStorage() {
      ...
      if (this.tokenExpiresAt) {
        localStorage.setItem('auth-tokenExpiresAt', this.tokenExpiresAt)
      } else {
        localStorage.removeItem('auth-tokenExpiresAt')
      }
